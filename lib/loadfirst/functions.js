Meteor.myFunctions = {
		...
		if (maxDim > zoomBox) {
			if (wDim >= zoomBox * 2) {
				...
				if (window.devicePixelRatio !== 1) {
					theFactor = 0.5;
					theWidth = wDim * theFactor;
					theHeight = hDim * theFactor;
				}
			}
		} else {
	...
	inverseHex(hex) {
		...
		// colorUp checks if a value is too close to the center
		// avoids an inverse output that is too similar to the input
		function colorUp(c) {
			let newC = 0;
			const offset = 96;
			const center = 128;
			const span = 16;
			const lowerThreshold = center - span;
			const upperTheshold = center + span;

			if (c > lowerThreshold && c < center) {
				newC = c + offset;
				return newC;
			}
			if (c < upperTheshold && c > center - 1) {
				newC = c - offset;
				return newC;
			}
			return c;
		...
		const rInput = parseInt(hex.substring(1, 3), 16);
		const gInput = parseInt(hex.substring(3, 5), 16);
		const bInput = parseInt(hex.substring(5, 7), 16);

		const r = (255 - colorUp(rInput)).toString(16);
		const g = (255 - colorUp(gInput)).toString(16);
		const b = (255 - colorUp(bInput)).toString(16);
	...
	// checks if tag has an alias and return key
	checkTagAliases(newTag) {
	...
	checkRelationAliases(newRelation) {
		const relAlias = Meteor.myFunctions.getNameOfValue(tagaliases, newRelation);
		if (relAlias) {
			return relAlias;
		}
		return newRelation;
